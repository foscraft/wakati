...
# Define valid tokens for access
ALBUM_TOKEN = "album_access_2025"
SINGLE_TOKEN = "single_access_2025"

# Determine page based on token
if access_token == ALBUM_TOKEN:
    page = "Download Wakati Wa Bwana"
elif access_token == SINGLE_TOKEN:
...
if page == "Home":
    ...
    # Generate and show both QR codes with longer URLs
    download_page_url = f"https://wakatiwabwana-album.streamlit.app/wakati-wa-bwana-full-album-access-2025/?token={ALBUM_TOKEN}"
    single_song_url = f"https://wakatiwabwana-album.streamlit.app/wakati-wa-bwana-single-track-access-2025/?token={SINGLE_TOKEN}"
    ...
    # Display access denied message if token is invalid
    st.markdown('<div class="main">', unsafe_allow_html=True)
    st.markdown('<h3 class="title">Access Denied</h3>', unsafe_allow_html=True)
    st.markdown('<p class="subtitle">Please scan the appropriate QR code to access this content.</p>', unsafe_allow_html=True)
    st.markdown('<p class="footer">Presented by Imara Daima Youth Choir © 2025</p>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)